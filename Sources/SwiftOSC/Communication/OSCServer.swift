...
open class OSCServer {
    ...
    open private(set) var running = false
                            ...
                            for char in type {
                                ...
                                case "s": // string
                                    ...
                                    messageData = messageData
                                        .subdata(in: (stringEnd / 4 + 1) * 4 ..< messageData.count)
                                case "b": // blob
        ...
        DispatchQueue.main.async {
            ...
            if let bundle = element as? OSCBundle {
                ...
                if bundle.timetag.secondsSinceNow < 0 {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + bundle.timetag.secondsSinceNow) {
                        self.delegate?.didReceive(bundle)
                        for element in bundle.elements {
                            self.sendToDelegate(element)
                        }
